...
import { Award,Briefcase,GraduationCap,Languages,Mail,MapPin,Phone,ShieldCheck } from 'lucide-react';
...
export function ModernSalesTemplate({ data, editable, onDataChange, sectionOrder, hiddenSections, showSectionToolbar, onSectionAction }: TemplateProps) {
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-8">
                            {projects.map((proj, idx) => (
                                <CVItemWrapper key={proj.id} onRemove={() => e.arrayHelpers.removeProject(idx)} editable={editable}>
                                <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col justify-center text-center hover:shadow-md transition-shadow">
        ...
        <div className="w-full bg-white min-h-[1000px] grid grid-cols-12 text-slate-800 font-sans">
            <div className="col-span-4 bg-[#0fa3b1] text-white p-6 space-y-8 flex flex-col h-full">
                <div className="w-48 h-48 mx-auto bg-white rounded-3xl overflow-hidden shadow-lg p-1">
                    {e.avatarField("rounded-2xl", { size: "w-full h-full" })}
                </div>
                <div className="text-center">
                    <h1 className="text-2xl font-bold uppercase mb-1">{e.personalField('fullName')}</h1>
                    <div className="text-emerald-100 uppercase tracking-widest text-xs font-semibold">{e.personalField('title')}</div>
                </div>
                <section>
                    <h3 className="text-lg font-bold uppercase border-b border-emerald-400 pb-2 mb-4 flex items-center gap-2">
                        Thông tin liên hệ
