import { CandidateProfile } from "./profile";

/**
 * Empty profile fixture - for new users or testing empty states
 * Shows 0% completion in ProfileCompletionWidget
 */
export const emptyProfile: CandidateProfile = {
  id: "empty-1",
  fullName: "",
  lastName: "",
  headline: "",
  title: "",
  email: "",
  phone: "",
  location: "",
  country: "",
  state: "",
  city: "",
  gender: "",
  linkedIn: "",
  website: "",
  avatarUrl: "",
  about: "",
  summary: "",
  yearsOfExperience: 0,
  expectedSalary: "",
  skills: [],
  experiences: [],
  educations: [],
  certificates: [],
  projects: [],
  languages: [],
  socialLinks: [],
  jobPreference: {
    jobTitles: [],
    preferredLocations: [],
    preferredIndustry: "",
    employmentType: "",
    preferredExperienceLevel: "",
    companySize: "",
    workPreference: "",
    willingToRelocate: false,
    availabilityDate: "",
    salary: "",
  },
  equalOpportunity: {
    pronouns: "",
    disability: "",
    veteranStatus: "",
    ethnicity: "",
    sexualOrientation: "",
  },
  aiQuestions: [],
};

/**
 * Partial profile fixture - for testing intermediate states
 * Shows ~40% completion in ProfileCompletionWidget
 */
export const partialProfile: CandidateProfile = {
  id: "partial-1",
  fullName: "Trần Thị Bình",
  headline: "",
  title: "Junior Developer",
  email: "binh.tran@example.com",
  phone: "+84 909 123 456",
  location: "Hà Nội, Việt Nam",
  avatarUrl: "",
  about: "Mới bắt đầu sự nghiệp lập trình.",
  summary: "",
  yearsOfExperience: 1,
  expectedSalary: "",
  skills: [
    { id: "1", skillName: "JavaScript", proficiencyLevel: "Beginner" },
    { id: "2", skillName: "HTML/CSS", proficiencyLevel: "Intermediate" },
  ],
  experiences: [],
  educations: [
    {
      id: "1",
      degree: "Cử nhân",
      institution: "Đại học Công nghệ Hà Nội",
      fieldOfStudy: "Công nghệ thông tin",
      startDate: "2019",
      endDate: "2023",
    },
  ],
  certificates: [],
  projects: [],
  languages: [
    { id: "1", language: "Tiếng Việt", proficiency: "Native" },
  ],
  socialLinks: [],
};

/**
 * Filled profile fixture - for testing complete states
 * Shows 100% completion in ProfileCompletionWidget
 */
export const filledProfile: CandidateProfile = {
  id: "1",
  fullName: "Nguyễn Văn An",
  headline: "Kỹ sư Frontend Cao cấp | Chuyên gia React & Next.js",
  title: "Lập trình viên Frontend Cao cấp",
  email: "an.nguyen@example.com",
  phone: "+84 912 345 678",
  location: "TP. Hồ Chí Minh, Việt Nam",
  avatarUrl: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=400&h=400&fit=crop&crop=face",
  about: "Lập trình viên frontend đam mê với hơn 5 năm kinh nghiệm xây dựng các ứng dụng web hiện đại. Chuyên sâu về React, Next.js và TypeScript. Yêu thích việc tạo ra giao diện người dùng đẹp mắt, hiệu suất cao.",
  summary: "Kỹ sư Frontend Cao cấp giàu kinh nghiệm với lịch sử làm việc được chứng minh trong ngành công nghệ thông tin. Thành thạo React.js, Next.js và Điện toán đám mây.",
  yearsOfExperience: 5,
  expectedSalary: "25.000.000 - 40.000.000 VND",
  skills: [
    { id: "1", skillName: "React", proficiencyLevel: "Expert" },
    { id: "2", skillName: "TypeScript", proficiencyLevel: "Expert" },
    { id: "3", skillName: "Next.js", proficiencyLevel: "Advanced" },
    { id: "4", skillName: "Tailwind CSS", proficiencyLevel: "Expert" },
    { id: "5", skillName: "Node.js", proficiencyLevel: "Advanced" },
    { id: "6", skillName: "GraphQL", proficiencyLevel: "Intermediate" },
    { id: "7", skillName: "PostgreSQL", proficiencyLevel: "Intermediate" },
    { id: "8", skillName: "Docker", proficiencyLevel: "Beginner" },
  ],
  experiences: [
    {
      id: "1",
      title: "Lập trình viên Frontend Cao cấp",
      companyName: "TechCorp Việt Nam",
      startDate: "2022-01",
      location: "TP. Hồ Chí Minh",
      locationType: "Hybrid",
      employmentType: "Full-time",
      description: "Dẫn dắt phát triển frontend cho nền tảng SaaS doanh nghiệp. Xây dựng hệ thống thiết kế, cải thiện hiệu suất 40%, hướng dẫn các lập trình viên junior.",
    },
    {
      id: "2",
      title: "Lập trình viên Frontend",
      companyName: "StartupXYZ",
      startDate: "2019-06",
      endDate: "2021-12",
      location: "Làm việc từ xa",
      locationType: "Remote",
      employmentType: "Full-time",
      description: "Xây dựng ứng dụng web responsive sử dụng React và Redux. Phối hợp với designers để triển khai giao diện chính xác đến từng pixel.",
    },
    {
      id: "3",
      title: "Lập trình viên Junior",
      companyName: "Digital Agency",
      startDate: "2018-01",
      endDate: "2019-05",
      location: "Đà Nẵng",
      locationType: "Onsite",
      employmentType: "Full-time",
      description: "Phát triển landing pages và websites marketing. Học hỏi các công nghệ frontend hiện đại và best practices.",
    },
  ],
  educations: [
    {
      id: "1",
      degree: "Cử nhân Khoa học",
      institution: "Đại học Bách khoa TP. Hồ Chí Minh",
      fieldOfStudy: "Khoa học Máy tính",
      startDate: "2014",
      endDate: "2018",
    },
  ],
  certificates: [
    {
      id: "1",
      name: "AWS Certified Solutions Architect - Associate",
      issuingOrganization: "Amazon Web Services",
      issueDate: "2023-05",
      expirationDate: "2026-05",
      credentialUrl: "https://aws.amazon.com/certification/",
    },
    {
      id: "2",
      name: "Meta Frontend Developer Professional Certificate",
      issuingOrganization: "Coursera",
      issueDate: "2021-08",
      credentialUrl: "https://coursera.org",
    }
  ],
  projects: [
    {
      id: "1",
      projectName: "SmartHire AI Platform",
      startDate: "2023-01",
      description: "Nền tảng tuyển dụng toàn diện sử dụng AI để kết nối ứng viên với công việc phù hợp.",
      technologies: ["Next.js", "TypeScript", "OpenAI API", "Tailwind CSS"],
      link: "https://smarthire.ai"
    },
    {
      id: "2",
      projectName: "E-commerce Dashboard",
      startDate: "2021-05",
      endDate: "2021-09",
      description: "Bảng điều khiển quản trị để quản lý sản phẩm, đơn hàng và khách hàng.",
      technologies: ["React", "Redux", "Firebase"],
      link: "https://demo-dashboard.com"
    }
  ],
  languages: [
    { id: "1", language: "Tiếng Việt", proficiency: "Native" },
    { id: "2", language: "Tiếng Anh", proficiency: "Fluent" },
  ],
  socialLinks: [
    { platform: "LinkedIn", url: "https://linkedin.com" },
    { platform: "GitHub", url: "https://github.com" },
    { platform: "Website", url: "https://example.com" },
  ],
  country: "Việt Nam",
  state: "",
  city: "TP. Hồ Chí Minh",
  gender: "Nam",
  linkedIn: "https://linkedin.com/in/nguyenvanan",
  website: "https://nguyenvanan.dev",
  jobPreference: {
    jobTitles: ["Senior Frontend Developer", "Tech Lead", "Full-stack Developer"],
    preferredLocations: ["TP. Hồ Chí Minh", "Remote"],
    preferredIndustry: "Công nghệ thông tin",
    employmentType: "Full-time",
    preferredExperienceLevel: "Senior",
    companySize: "50-200",
    workPreference: "Hybrid",
    willingToRelocate: false,
    availabilityDate: "2026-03-01",
    salary: "35.000.000 - 50.000.000 VND",
  },
  equalOpportunity: {
    pronouns: "Anh ấy / He/Him",
    disability: "Không",
    veteranStatus: "Không",
    ethnicity: "Kinh",
    sexualOrientation: "",
  },
  aiQuestions: [
    {
      id: "1",
      question: "Mô tả một dự án mà bạn tự hào nhất. Hãy chia sẻ từ đầu đến cuối bao gồm động lực và chi tiết kỹ thuật.",
      answer: "Dự án SmartHire AI Platform — tôi đã xây dựng toàn bộ frontend từ đầu sử dụng Next.js 14, thiết kế hệ thống component library riêng, tích hợp OpenAI API cho tính năng phân tích CV tự động.",
    },
    {
      id: "2",
      question: "Bạn đang tìm kiếm điều gì ở vai trò tiếp theo? Điều gì bạn muốn tránh?",
      answer: "Tôi muốn làm việc trong một team nhỏ, năng động, có cơ hội mentoring junior. Tôi muốn tránh môi trường quá nhiều bureaucracy.",
    },
    {
      id: "3",
      question: "Thách thức lớn nhất khi làm việc nhóm và bạn đã vượt qua như thế nào?",
      answer: "",
    },
    {
      id: "4",
      question: "Vấn đề phức tạp nhất bạn từng giải quyết? Chia sẻ kỹ thuật và công cụ đã dùng.",
      answer: "",
    },
    {
      id: "5",
      question: "Bạn xử lý nhiều dự án cùng lúc như thế nào? Phương pháp quản lý thời gian?",
      answer: "",
    },
    {
      id: "6",
      question: "Sở thích và hoạt động ngoài công việc của bạn? Chúng ảnh hưởng thế nào đến cách bạn làm việc?",
      answer: "",
    },
  ],
};

/**
 * Default export - alias for filledProfile (backward compatibility)
 */
export const mockProfile = filledProfile;

/**
 * All fixtures for easy access
 */
export const profileFixtures = {
  empty: emptyProfile,
  partial: partialProfile,
  filled: filledProfile,
} as const;

export type ProfileFixtureType = keyof typeof profileFixtures;
