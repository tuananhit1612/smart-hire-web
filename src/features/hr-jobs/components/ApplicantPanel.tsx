...
function ApplicantRow({
    ...
    const statusColor: Record<string, string> = {
        ...
        interviewed: "bg-[#22c55e]/15 text-[#22c55e] border-[#22c55e]/30",
    ...
    return (
        <motion.div
            ...
            className={`relative p-4 rounded-2xl cursor-pointer transition-all duration-300 group ${isSelected
                    ? "bg-gradient-to-r from-[rgba(145,158,171,0.04)] via-white to-[#22c55e]/5 border-2 border-[#22c55e]/30 shadow-lg shadow-green-500/20 ring-2 ring-[#22c55e]/30"
                    : "bg-white/80 backdrop-blur-sm border-2 border-white/60 hover:border-[#22c55e]/30 hover:shadow-lg hover:shadow-green-500/20"
                }`}
        >
            {/* Selected glow */}
            {isSelected && (
                <motion.div
                    layoutId="activeGlow"
                    className="absolute -left-0.5 top-3 bottom-3 w-1 bg-gradient-to-b from-[#22c55e] via-[#10b981] to-green-400 rounded-full"
                />
            )}
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                        <h4 className="font-bold text-sm text-[#1C252E] truncate">{applicant.name}</h4>
                    ...
                    <div className="flex items-center gap-3 mt-1.5 text-[11px] text-[#22c55e]">
            ...
            <div className="flex flex-wrap gap-1 mt-2 pl-[60px]">
                {applicant.skills.slice(0, 3).map((skill) => (
                    <span key={skill} className="px-2 py-0.5 rounded-lg text-[10px] font-medium bg-[#22c55e]/10 text-[#22c55e] border border-[rgba(145,158,171,0.12)]">
                        {skill}
                    </span>
                ))}
                {applicant.skills.length > 3 && (
                    <span className="px-2 py-0.5 bg-[#22c55e]/10 text-[#22c55e] rounded-lg text-[10px]">
                        +{applicant.skills.length - 3}
                    </span>
                )}
            </div>
    ...
    return (
        <div className="space-y-5 p-6">
            ...
            <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="bg-gradient-to-r from-[#22c55e] via-[#10b981] to-[#10b981] rounded-3xl p-6 text-white relative overflow-hidden shadow-xl shadow-green-500/20"
            >
                ...
                <div className="relative flex items-center gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[#22c55e] font-semibold text-sm mt-0.5 uppercase tracking-wider">{personalInfo.title || "Ứng viên"}</p>
            ...
            {summary && (
                <motion.div
                    ...
                    className="bg-white/80 backdrop-blur-xl rounded-2xl p-5 border border-[rgba(145,158,171,0.12)] shadow-lg shadow-[#22c55e]/20"
                >
                    <h3 className="text-sm font-bold text-[#22c55e] uppercase tracking-wider mb-3 flex items-center gap-2">
                        <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-[#22c55e] to-[#10b981] flex items-center justify-center">
                            <FileText className="w-3.5 h-3.5 text-white" />
                        </div>
                        Giới thiệu bản thân
                    </h3>
                    <p className="text-sm text-[#1C252E] leading-relaxed whitespace-pre-line">{summary}</p>
                </motion.div>
            )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
                ...
                {experience.length > 0 && (
                    <motion.div
                        ...
                        className="xl:col-span-3 bg-white/80 backdrop-blur-xl rounded-2xl p-5 border border-[rgba(145,158,171,0.12)] shadow-lg shadow-[#22c55e]/20"
                    >
                        ...
                        <div className="space-y-4">
                            {experience.map((exp, i) => (
                                <motion.div
                                    ...
                                    className="relative pl-4 border-l-2 border-[#22c55e]/30 hover:border-[#22c55e]/30 transition-colors group"
                                >
                                    <div className="absolute -left-[5px] top-1.5 w-2 h-2 rounded-full bg-[rgba(145,158,171,0.04)] group-hover:bg-[#22c55e] transition-colors ring-2 ring-white" />
                                    <div className="flex items-baseline justify-between gap-2 mb-1">
                                        <h4 className="font-bold text-[#1C252E] text-sm group-hover:text-[#22c55e] transition-colors">{exp.position}</h4>
                                        <span className="text-[10px] text-[#22c55e] font-semibold bg-[#22c55e]/10 px-2 py-0.5 rounded-full flex-shrink-0">
                                            {exp.startDate} — {exp.isCurrent ? "Hiện tại" : exp.endDate}
                                        </span>
                                    </div>
                                    <p className="text-xs text-[#22c55e] font-semibold mb-2">{exp.company}</p>
                                    <p className="text-xs text-[#1C252E] leading-relaxed whitespace-pre-line">{exp.description}</p>
                                </motion.div>
                            ))}
                        </div>
                    </motion.div>
                ...
                {education.length > 0 && (
                    <motion.div
                        ...
                        className="xl:col-span-2 bg-white/80 backdrop-blur-xl rounded-2xl p-5 border border-[rgba(145,158,171,0.12)] shadow-lg shadow-[#22c55e]/20"
                    >
                        <h3 className="text-sm font-bold text-[#22c55e] uppercase tracking-wider mb-4 flex items-center gap-2">
                        ...
                        <div className="space-y-4">
                            {education.map((edu, i) => (
                                <motion.div
                                    ...
                                    className="p-3 bg-gradient-to-r from-[rgba(145,158,171,0.04)] to-white rounded-xl border border-[rgba(145,158,171,0.12)] hover:border-[#22c55e]/30 transition-all hover:shadow-md"
                                >
                                    <h4 className="font-bold text-[#1C252E] text-sm">{edu.school}</h4>
                                    <p className="text-xs text-[#22c55e] font-semibold mt-0.5">{edu.degree}</p>
                                    <p className="text-[11px] text-[#22c55e] mt-0.5">{edu.field}</p>
                                    <p className="text-[10px] text-[#22c55e] mt-1">{edu.startDate} — {edu.endDate}</p>
                                    {edu.description && (
                                        <p className="text-[11px] text-[#22c55e] mt-2 whitespace-pre-line">{edu.description}</p>
                                    )}
                                </motion.div>
                            ))}
                        </div>
            ...
            {skills.length > 0 && (
                ...
                >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {/* Technical */}
                        <div>
                            <p className="text-[10px] font-bold text-[#22c55e] uppercase tracking-wider mb-2">Chuyên môn</p>
                            <div className="flex flex-wrap gap-2">
                                {skills.filter(s => s.category === "technical").map((s) => (
                                    <motion.span
                                        key={s.id}
                                        whileHover={{ scale: 1.05 }}
                                        className="px-3 py-1.5 bg-gradient-to-r from-[rgba(145,158,171,0.04)] to-[#22c55e]/5 text-[#22c55e] rounded-xl text-xs font-semibold border border-[#22c55e]/30 hover:border-[#22c55e]/30 hover:shadow-md transition-all cursor-default"
                                    >
                                        {s.name}
                                    </motion.span>
                                ))}
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        {projects.map((p, i) => (
                            <motion.div
                                ...
                                whileHover={{ y: -2 }}
                                className="p-4 bg-gradient-to-r from-[rgba(145,158,171,0.04)] to-white rounded-xl border border-[rgba(145,158,171,0.12)] hover:border-[#22c55e]/30 hover:shadow-lg transition-all"
                            >
                                <h4 className="font-bold text-[#1C252E] text-sm">{p.name}</h4>
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                    {p.technologies.map((t) => (
                                        <span key={t} className="text-[10px] px-2 py-0.5 bg-white border border-[rgba(145,158,171,0.12)] rounded-lg text-[#22c55e] font-semibold shadow-sm">{t}</span>
                                    ))}
                                </div>
                                <p className="text-xs text-[#1C252E] mt-2.5 leading-relaxed">{p.description}</p>
                            </motion.div>
                        ))}
                    </div>
                </motion.div>
...
function CVDetailView({
    ...
    return (
        <div className="h-full flex flex-col">
            {/* Top Action Bar */}
            <div className="flex items-center gap-3 px-6 py-3.5 border-b border-[rgba(145,158,171,0.12)] bg-white/80 backdrop-blur-xl flex-shrink-0">
                <div className="flex items-center gap-3 flex-1">
                    <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#22c55e] to-[#10b981] flex items-center justify-center text-white text-sm font-bold shadow-lg shadow-green-500/20">
                        {applicant.name.charAt(0)}
                    </div>
                    <div>
                        <h3 className="font-bold text-[#1C252E] text-sm">{applicant.name}</h3>
                        <p className="text-[11px] text-[#22c55e]">{applicant.currentCompany || "Freelancer"} • {applicant.experience}</p>
                    </div>
                </div>

                {/* Contact shortcuts */}
                <a href={`mailto:${applicant.email}`}
                    className="flex items-center gap-1.5 px-3 py-2 bg-[#22c55e]/10 text-[#22c55e] rounded-xl text-xs font-semibold hover:bg-[#22c55e]/15 border border-[rgba(145,158,171,0.12)] hover:border-[#22c55e]/30 transition-all hover:scale-105">
                ...
                <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 bg-white text-[#22c55e] rounded-xl text-xs font-semibold border-2 border-[#22c55e]/30 hover:border-[#22c55e]/30 hover:bg-[#22c55e]/10 transition-all hover:scale-105">
            ...
            {/* CV Content */}
            <div className="flex-1 overflow-y-auto bg-gradient-to-br from-[rgba(145,158,171,0.04)] via-white to-[#22c55e]/5">
                {applicant.cvData ? (
...
function FallbackCVView({ applicant }: { applicant: Applicant }) {
    return (
        <div className="p-6 space-y-5">
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-5 border border-[rgba(145,158,171,0.12)] shadow-lg shadow-[#22c55e]/20">
                ...
                <div className="flex flex-wrap gap-2">
                    {applicant.skills.map((skill) => (
                        <span key={skill} className="px-4 py-2 bg-gradient-to-r from-[rgba(145,158,171,0.04)] to-[#22c55e]/5 text-[#22c55e] rounded-xl text-sm font-semibold border border-[#22c55e]/30">{skill}</span>
                    ))}
                </div>
            </div>
        </div>
...
function EmptyCVState() {
    return (
        <div className="h-full flex flex-col items-center justify-center bg-gradient-to-br from-[rgba(145,158,171,0.04)] via-white to-[#22c55e]/5">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
                <motion.div
                    animate={{ y: [-6, 6, -6] }}
                    transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
                    className="w-24 h-24 mx-auto mb-5 rounded-3xl bg-gradient-to-br from-[rgba(145,158,171,0.04)] to-[#22c55e]/5 flex items-center justify-center shadow-xl shadow-green-500/20"
                >
                    <FileText className="w-12 h-12 text-[#22c55e]" />
                </motion.div>
                <h3 className="text-xl font-bold text-[#1C252E] mb-2">Xem CV ứng viên</h3>
                <p className="text-[#22c55e] max-w-xs mx-auto text-sm">
                    Chọn một ứng viên từ danh sách bên trái để xem hồ sơ chi tiết
                </p>
                <motion.div
                    animate={{ x: [-3, 3, -3] }}
                    transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                    className="flex items-center gap-2 mt-5 text-[#22c55e] justify-center"
                >
...
export function ApplicantPanel({ job, onClose }: { job: Job; onClose: () => void }) {
    ...
    return (
        <motion.div
            ...
            className="fixed inset-0 bg-[#1C252E] backdrop-blur-sm z-50 flex items-center justify-center p-3"
            onClick={onClose}
        >
            <motion.div
                ...
                className="w-full max-w-[92vw] h-[90vh] bg-white/95 backdrop-blur-2xl rounded-3xl shadow-2xl shadow-[#22c55e]/20 overflow-hidden flex border border-[rgba(145,158,171,0.12)]"
            >
                {/* ═══ LEFT PANEL: Applicant List ═══ */}
                <div className="w-[380px] flex-shrink-0 flex flex-col bg-gradient-to-b from-white to-[#22c55e]/5 border-r border-[rgba(145,158,171,0.12)]">
                    {/* Header */}
                    <div className="relative overflow-hidden flex-shrink-0">
                        <div className="absolute inset-0 bg-gradient-to-br from-[#22c55e] via-[#10b981] to-[#10b981]" />
                        <div className="absolute inset-0 opacity-10">
                    ...
                    <div className="p-3 space-y-2 flex-shrink-0">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-3 py-2.5 bg-white border-2 border-[rgba(145,158,171,0.12)] rounded-2xl focus:border-[#22c55e]/30 focus:ring-2 focus:ring-[#22c55e]/30 focus:outline-none transition-all text-sm text-[#1C252E] placeholder:text-[#22c55e]"
                            />
                        </div>
                        <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
                            <button onClick={() => setFilter("all")}
                                className={`px-3 py-1.5 rounded-full text-[11px] font-bold whitespace-nowrap transition-all ${filter === "all"
                                        ? "bg-gradient-to-r from-[#22c55e] to-[#10b981] text-white shadow-md shadow-green-500/20"
                                        : "bg-white text-[#22c55e] border border-[#22c55e]/30 hover:border-[#22c55e]/30 hover:scale-105"
                                    }`}>
                        ...
                        <AnimatePresence mode="popLayout">
                            {filteredApplicants.length > 0 ? (
                                ...
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-10">
                                    <div className="w-14 h-14 mx-auto mb-3 rounded-2xl bg-[#22c55e]/10 flex items-center justify-center">
                                        <User className="w-7 h-7 text-[#22c55e]" />
                                    </div>
                                    <h3 className="text-sm font-bold text-[#1C252E] mb-1">Không tìm thấy</h3>
                                    <p className="text-[#22c55e] text-xs">{searchValue ? "Thử từ khóa khác" : "Chưa có ứng viên"}</p>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>
